...
// determine date range of messages

// emails are in one string, delimted by ##||##
// emails have 5 parts delimted by #/#
// parts are sender, subject, date, recipient, body
// care about the dates only
  let dates = emailData.split('##||##').map(message => {
    // date is the third part of the message (hence 2)
    // the match for the Regex is at 0
    let messageParts = message.split('#/#')
    return new Date(messageParts[2].match(/\d\d-\d\d-\d\d\d\d/)[0]);
  });

  dates = dates.sort((date1, date2) => date1 < date2 ? -1 : 1);
