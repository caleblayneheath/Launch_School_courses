class TTTGame
  ...
  def display_game_result
    message = if computer.score == human.score
                "It's a tie!"
...
class Human < Player
  ...
  def choose_first_mover(computer_marker)
    input = nil
    loop do
      ...
      break if [marker, computer_marker].include?(input)
      puts "Invalid entry."
    end
    input
  end
...
class Computer < Player
  ...
  def moves(board, human_marker, computer_marker)
    ...
    center = [5] if board.center_unmarked?

    moves = squares_where_computer_wins || squares_where_human_wins ||
            center || board.unmarked_keys
    square = moves.sample
